...
_ = transfer_model(np.zeros((1, 224, 224, 3), dtype=np.float32), training=False)
...
def predict():
    try:
        ...
        img_size = (224, 224) if model_type=="transfer" else (64,64)
        ...
        confidence = round(float(np.max(preds[0]) * 100), 1)
        ...
        cursor.execute("""
            ...
            VALUES (%s, %s, %s, %s, %s, %s)
        """, (user_id, predicted_label, confidence, filepath, heatmap_path, model_type))
        db.commit()
        ...
        cursor.execute("""
            ...
            VALUES (%s, %s, %s)
        """, (user_id, predicted_label, confidence))
        db.commit()
